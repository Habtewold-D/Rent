...
import '../landlord/landlord_verification_page.dart';
...
class _RenterProfilePageState extends State<RenterProfilePage> {
  ...
  // Religion uses dropdown against backend enum values
  ...
  Widget build(BuildContext context) {
    ...
    return ChangeNotifierProvider(
      ...
      builder: (context, _) {
        ...
        final role = user['role']?.toString() ?? 'renter';
        final status = vm.landlordStatus ?? const {};
        final isVerified = status['isVerified'] == true;
        final isLandlord = status['isLandlord'] == true;
        final canRequestVerification = status['canRequestVerification'] != false;
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Wrap(
                      ...
                      runSpacing: 8,
                      children: [
                        _Chip(label: 'Role: $role', color: Colors.blueGrey.shade100),
                        if (isVerified)
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          Text('Landlord verification', style: Theme.of(context).textTheme.titleMedium),
                        ],
                      ),
                      const SizedBox(height: 12),
                      Wrap(
                        ...
                        children: [
                          if (isVerified)
                            _Chip(label: 'Verified', color: Colors.green.shade100, icon: Icons.verified)
                          else if (isLandlord)
                            _Chip(label: 'Pending review', color: Colors.orange.shade100, icon: Icons.schedule)
                          else
                            _Chip(label: 'Not a landlord', color: Colors.grey.shade200),
                        ],
                      ),
                      const SizedBox(height: 12),
                      if (!isVerified && canRequestVerification)
                        SizedBox(
                          width: double.infinity,
                          child: OutlinedButton.icon(
                            ...
                            label: const Text('Become a landlord'),
                          ),
                        ),
                    ],
                  ),
